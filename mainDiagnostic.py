...
class SelfExcitingPairsTrading:
    ...
    def run_complete_pipeline(self):
        ...
        print("NVDA/AMD Equity Pairs Strategy")
        print("=" * 70)
    ...
    def _generate_signals(self):
        ...
        print(f"\n{'='*70}")
        print(f"🔍 DETAILED SIGNAL GENERATION DIAGNOSTIC")
        print(f"{'='*70}")
        
        print(f"\n⚙️  Configuration:")
        print(f"  z_entry_threshold: {cfg.z_entry_threshold}")
        print(f"  lambda_threshold: {cfg.lambda_threshold}")
        
        print(f"\n📊 Data Dimensions:")
        print(f"  Total observations: {len(z_score)}")
        ...
        print(f"\n🌊 Lambda Statistics:")
        print(f"  Range: [{hawkes_intensity.min():.6f}, {hawkes_intensity.max():.6f}]")
        print(f"  Mean: {hawkes_intensity.mean():.6f}")
        
        print(f"\n🎯 Entry Condition Analysis:")
        z_below = (z_score < -cfg.z_entry_threshold)
        z_above = (z_score > cfg.z_entry_threshold)
        lambda_ok = (hawkes_intensity < cfg.lambda_threshold)
        
        print(f"  Z < -{cfg.z_entry_threshold}: {z_below.sum()} times ({100*z_below.sum()/len(z_score):.2f}%)")
        print(f"  Z > {cfg.z_entry_threshold}: {z_above.sum()} times ({100*z_above.sum()/len(z_score):.2f}%)")
        print(f"  λ < {cfg.lambda_threshold}: {lambda_ok.sum()} times ({100*lambda_ok.sum()/len(hawkes_intensity):.2f}%)")
        
        long_cand = (z_below & lambda_ok).sum()
        short_cand = (z_above & lambda_ok).sum()
        print(f"\n✅ Combined Candidates:")
        print(f"  LONG: {long_cand}, SHORT: {short_cand}, TOTAL: {long_cand + short_cand}")
        
        if long_cand + short_cand < 10:
            print(f"\n❌ PROBLEM: Only {long_cand + short_cand} entry candidates!")
            if z_below.sum() + z_above.sum() < 20:
                print(f"   → Z-scores rarely exceed threshold (check sigma_X calculation)")
            elif lambda_ok.sum() < len(hawkes_intensity) * 0.5:
                print(f"   → Lambda threshold too restrictive")
        
        # Show MRJD params used for Z-score
        mrjd_p = self.mrjd_model.params
        sigma_X = mrjd_p['sigma'] / np.sqrt(2 * mrjd_p['kappa'])
        print(f"\n🔬 MRJD σ_X: {sigma_X:.4f}")
        print(f"  For Z=±{cfg.z_entry_threshold}, spread needs |deviation| > {cfg.z_entry_threshold * sigma_X:.2f}")
        print(f"  Actual spread range: [{spread.min():.2f}, {spread.max():.2f}]")
    ...
    def _run_backtest(self):
        ...
        self.equity_curve = self.backtest_engine.run_backtest(
            signals_df,
            spread_df,
            asset_a_prices,  # NVDA prices
            asset_b_prices,  # AMD prices
